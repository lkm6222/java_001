		...
		while(true) {
			...
			switch(choice){
				...
				case 0:
					...
					return;
				default:
					System.out.println("0~2까지의 번호만 선택해주세요.");
			}
		}
...
/*
		// 고민해결책의 답 가져오기		
		ArrayList<String> solutionBook =  forYouDAO.selectSolutionBook();

		System.out.println("<" + name + ">님의 고민에 대한 저의 대답이에요.");
		System.out.println("-------------------------");

		// 상세 로직 구현
		int selectPage = (int) (Math.random() * solutionBook.size());
		String result = solutionBook.get(selectPage);
		
		System.out.println("[ 고민해결 ]" + "\n" + result);
		System.out.println("-------------------------");		
*/
		System.out.println("<" + name + ">님의 고민에 대한 저의 대답이에요.");
		System.out.println("-------------------------");
...
/*
		// 고민해결책의 답 가져오기
		ArrayList<String> fortuneBook = forYouDAO.selectFortuneBook();
		...
		// 상세 로직 구현
		int selectPage = (int) (Math.random() * fortuneBook.size());
		String result = fortuneBook.get(selectPage);
		
		System.out.println("[ 행운메세지 ]" + "\n" + result);
		System.out.println("-------------------------");
*/
